...
the group for an element in the array and returns an object that maps
...
const ageDiff = (array) => {
  return array.died - array.born;
}

const group = (array, func) => {
  let obj = {};
  array.forEach((person) => {
    let century = func(person);
    if (century in obj) {
      obj[century].push(ageDiff(person));
    } else {
      obj[century] = [ageDiff(person)];
    }
  });
  return obj;
}

let data = group(ancestry,(val) => {
  return Math.round( val.died / 100 );
});
